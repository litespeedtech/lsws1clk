...
MARIADBVERLIST=(10.6 10.11 11.4 11.6 11.8)
LSPHPVER=84
MARIADBVER=11.8
#MYSQLVER=8.0
LICENSE='TRIAL'
...
function usage
{
    ...
    echoNW "  -L,    license"                 "${EPACE}    Uses specified LSWS serial number."    
    echoW " --adminuser [USERNAME]"           "${EPACE}    Sets the WebAdmin username for LiteSpeed instead of admin."
    echoNW "  -A,    --adminpassword [PASSWORD]" "${EPACE}Sets the WebAdmin password for LiteSpeed instead of using a random one."
    echoW " --adminport [PORTNUMBER]"           "${EPACE}  Sets the WebAdmin console port number instead of 7080."
    echoNW "  -E,    --email [EMAIL]          " "${EPACE} Sets the administrator email."
    echoW " --lsphp [VERSION]                 " "Sets the LSPHP version, such as 83. We currently support versions '${LSPHPVERLIST[@]}'."
    echoW " --mariadbver [VERSION]            " "Sets MariaDB version, such as 11.4. We currently support versions '${MARIADBVERLIST[@]}'."
    echoNW "  -W,    --wordpress              " "${EPACE} Installs WordPress. You will still need to complete the WordPress setup by browser"
    echoW " --wordpressplus [SITEDOMAIN]      " "Installs, setup, and configures WordPress, also LSCache will be enabled"
    echoW " --wordpresspath [WP_PATH]         " "Sets a location for the new WordPress installation or an existing WordPress."
    echoNW "  -R,    --dbrootpassword [PASSWORD]  " "     Sets the database root password instead of using a random one."
    echoW " --dbname [DATABASENAME]           " "Sets the database name to be used by WordPress."
    echoW " --dbuser [DBUSERNAME]             " "Sets the WordPress username in the database."
    echoW " --dbpassword [PASSWORD]           " "Sets the WordPress table password in MySQL instead of using a random one."
    echoW " --prefix [PREFIXNAME]             " "Sets the WordPress table prefix."
    echoW " --user [PORT]                     " "Sets the user that LiteSpeed will run as. Default is nobody."
    echoW " --group [PORT]                    " "Sets the group that LiteSpeed will run as."    
    echoW " --listenport [PORT]               " "Sets the HTTP server listener port, default is 80."
    echoW " --ssllistenport [PORT]            " "Sets the HTTPS server listener port, default is 443."
    echoW " --wpuser [WORDPRESSUSER]          " "Sets the WordPress admin user for WordPress dashboard login. Default value is wpuser."
    echoW " --wppassword [PASSWORD]           " "Sets the WordPress admin user password for WordPress dashboard login."
    echoW " --wplang [WP_LANGUAGE]            " "Sets the WordPress language. Default value is \"en_US\" for English."
    echoW " --sitetitle [WP_TITLE]            " "Sets the WordPress site title. Default value is mySite."
    echoW " --sitedomain [SITEDOMAIN]         " "Sets domain name mapping on listener level."     
    echoW " --pure-mariadb                    " "Installs LiteSpeed and MariaDB"
    echoW " --pure-mysql                      " "Installs LiteSpeed and MySQL"
    echoW " --pure-percona                    " "Installs LiteSpeed and Percona"
    echoW " --with-mysql                      " "Installs LiteSpeed/App with MySQL"
    echoW " --with-percona                    " "Installs LiteSpeed/App with Percona"
    echoW " --owasp-enable                    " "Enables mod_security with OWASP rules. If lsws is installed, then enable the owasp directly"
    echoW " --owasp-disable                   " "Disables mod_security with OWASP rules."
    echoW " --fail2ban-enable                 " "Enables fail2ban for webadmin and wordpress login pages"    
    echoW " --proxy-r                         " "Sets a proxy with rewrite type."
    echoW " --proxy-c                         " "Sets a proxy with config type."
    echoW " --proxy-port [PORT]               " "Sets a proxy port, default value is 8080."
    echoNW "  -U,    --uninstall              " "${EPACE} Uninstalls LiteSpeed and remove installation directory."
    echoNW "  -P,    --purgeall               " "${EPACE} Uninstalls LiteSpeed, remove installation directory, and purge all data in MySQL."
    echoNW "  -Q,    --quiet                  " "${EPACE} Uses quiet mode, won't prompt to input anything."
    echoNW "  -V,    --version                " "${EPACE} Displays the script version information."
    echoNW "  -v,    --verbose                " "${EPACE} Displays more messages during the installation."
    echoW " --update                          " "Updates lsws1clk from github."
    echoNW "  -H,    --help                   " "${EPACE} Displays help messages."
    echo 
    echo -e "\033[1mEXAMPLES\033[0m"
    echoW "./lsws1clk.sh                       " "Installs LiteSpeed with a random WebAdmin password."
    echoW "./lsws1clk.sh --lsphp 83            " "Installs LiteSpeed with lsphp83."
    echoW "./lsws1clk.sh -A 123456 -e a@cc.com " "Installs LiteSpeed with WebAdmin password  \"123456\" and email a@cc.com."
    echoW "./lsws1clk.sh -R 123456 -W          " "Installs LiteSpeed with WordPress and MariaDB root password \"123456\"."
    echoW "./lsws1clk.sh --wordpressplus a.com " "Installs LiteSpeed with a fully configured WordPress installation at \"a.com\"."
    echo
    exit 0
}
